...
void CProbeDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!pDX->m_bSaveAndValidate) // data to dialog
	{
		...
		//todo1.AppendFormat("usemask? = %d (%04X)\r\n",pprobe->stats_ss1,pprobe->stats_ss1);
		//todo1.AppendFormat("ss2 = %d (%04X)\r\n",pprobe->stats_ss2,pprobe->stats_ss2);
		//todo1.AppendFormat("ss3 = %d (%04X)\r\n",pprobe->stats_ss3,pprobe->stats_ss3);
		/*
		todo1.AppendFormat("\r\nTODO1:\r\n");
		for (int i=0;i<sizeof(pprobe->TODO1);i++)
		{
			todo1.AppendFormat("%02X ",pprobe->TODO1[i]);
			if (!((i+1)%16)) todo1.AppendFormat("\r\n");
		}
		
		todo1.AppendFormat("\r\nTODO3:\r\n");
		for (int i=0;i<sizeof(pprobe->TODO3);i++)
		{
			todo1.AppendFormat("%02X ",pprobe->TODO3[i]);
			if (!((i+1)%16)) todo1.AppendFormat("\r\n");
		}
		*/
		todo1.AppendFormat("\r\nEND");
