...
void CProjectileDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!pDX->m_bSaveAndValidate) // data to dialog
	{
		...
		/*
		weplist.AppendFormat("\r\npad2=");
		for (int i=0;i<2;i++)
			weplist.AppendFormat("%02X ",pprojectile->pad2[i]);
		weplist.AppendFormat("\r\npad3=");
		for (int i=0;i<3;i++)
			weplist.AppendFormat("%02X ",pprojectile->pad3[i]);
		weplist.AppendFormat("\r\npad4=");
		for (int i=0;i<2;i++)
			weplist.AppendFormat("%02X ",pprojectile->pad4[i]);
		weplist.AppendFormat("\r\nheader=");
		*/
		DDX_Text(pDX, IDC_PROJWEP,weplist);
	}
