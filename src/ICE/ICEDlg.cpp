...
BOOL CICEDlg::OnInitDialog()
{
	...
	dlgDrone.Create(IDD_DRONEDLG,this);
	...
	dlgProjectile.sArtPath = cArtPath;
	dlgShip.sArtPath = cArtPath;
	dlgShipLoadout.sArtPath = cArtPath;
	dlgStation.sArtPath = cArtPath;
	dlgProbe.sArtPath = cArtPath;
	dlgMissile.sArtPath = cArtPath;
	dlgCiv.sArtPath = cArtPath;
	if (sCoreArg != "")
	{
...
CString CICEDlg::DoBackup(CString file, CString name)
{
	...
	CString bkfile;
	CFileStatus dummy;
	for (int backnum=1;backnum<1000;backnum++)
	{
...
void CICEDlg::ShowTechEntry(UCHAR *pTT,UCHAR *pTTLocal)
{
	...
	if (pTT)
	{
		...
		if (sTechName.Left(7) == "Faction")
			t1.Format("%s\r\nFaction warning: Pre is Def and Def='No devel' defaults",tb);
		else
			t1.Format("%s\r\n",tb);
	}
	if (pTTLocal)
