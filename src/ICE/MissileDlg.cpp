...
void CMissileDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!pDX->m_bSaveAndValidate) // data to dialog
	{
		...
		type = pmissile->type;
		uid = pmissile->uid;
		cbse->SetCurSel(pmissile->special_effect);
	...
	DDX_Text(pDX, IDC_DAMAGE, pmissile->stats_damage);
	DDX_Text(pDX, IDC_DAMAGE_RADIUS, pmissile->stats_damage_radius);
	...
	if (pDX->m_bSaveAndValidate) // dialog to data
	{
		...
		pmissile->special_effect = cbse->GetCurSel();
...
BOOL CMissileDlg::OnInitDialog(void)
{
	...
	for (int  i=0;i<IGCMISSILE_EFFECT_NBVALS;i++)
		cbse->AddString(ICGMissileEffects[i]);
