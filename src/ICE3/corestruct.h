...
"lt & med shield","minor base shld","major base shld","parts",
...
{
	float ukf[2]; // starting money modifier
	char name[25];
	char model[13];
	char obj[13];
	UCHAR techtree[101];// first = 02
	float factors[25];
	unsigned short suk; // lifepod ID
	unsigned short uid;
...
{
	"Counter",	"undef 1",
		"Missile", "Pack","Shield","Cloak","undef 6","Afterburner",
};
...
#define IGCMISSILE_EFFECT_NBVALS   3
...
typedef struct SIGCCoreMine // tag = 0x18, size = 480
{
	...
	float stats_s5; // mass
	unsigned short stats_ss1; //usemask
