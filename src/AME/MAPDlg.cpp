...
void CMAPDlg::LoadSectors()
{
	...
	for (i=0;i < pigcmap->cl_alephs.GetCount();i++)
	{
		CIGCAleph *paleph;
	...
	for (i=0;i < pigcmap->cl_bases.GetCount();i++)
	{
		CIGCBase *pbase;
	...
	for (i=0;i < pigcmap->cl_rocks.GetCount();i++)
	{
		CIGCRock *prock;
...
void CMAPDlg::OnClickedMapLoad(void)
{
	...
	CFileDialog cfd(TRUE,"igc",NULL,OFN_FILEMUSTEXIST|OFN_NOCHANGEDIR|OFN_NONETWORKBUTTON,"Allegiance Map|*.igc|",this,OPENFILENAME_SIZE_VERSION_400);
