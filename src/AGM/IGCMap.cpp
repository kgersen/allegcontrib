...
// thorium  = bgrnd55 = 8 / goldrock = 4
	...
	{
		igcrock.he3_1 = igcrock.radius * 2.0f; 
		igcrock.he3_2 = igcrock.radius * 2.0f; 
	}
	RandomOrientation();
	RandomSpin();	
}
...
BOOL CIGCMap::SaveToFile(CString fn)
{
	...
	for (int i=0;i < cl_alephs.GetCount();i++)
	{
		...
		cfmap.Write(&tag,sizeof(tag));		// the tag
		cfmap.Write(&size,sizeof(size));    // the size
		cfmap.Write(&paleph->igcaleph,size);  // the struct
	}
...
int CIGCAleph::DispProps(CWnd * pParent,PtrCore pcore)
{
#ifdef _AGM
	CAlephDlg adlg;
	adlg.curaleph = this;
...
int CIGCBase::DispProps(CWnd * pParent,PtrCore pcore)
{
#ifdef _AGM
	CBaseDlg bdlg;
	bdlg.curbase = this;
...
int CIGCRock::DispProps(CWnd * pParent,PtrCore pcore)
{
#ifdef _AGM
	CRockDlg rdlg;
	rdlg.currock = this;
...
void CIGCMap::FixUIDs()
{
	...
	pos = cl_alephs.GetHeadPosition();
	for (i=0;i < cl_alephs.GetCount();i++)
	{
...
// set alephs size
void CIGCMap::SetAlephsSize(unsigned short size)
{
		POSITION pos = cl_alephs.GetHeadPosition();
		for (int i=0;i < cl_alephs.GetCount();i++)
		{
			PtrAleph paleph =  &(cl_alephs.GetNext(pos));
			paleph->igcaleph.sig = size/100;
			paleph->igcaleph.radius = size;
		}
}
CString CIGCMap::CheckValidity(void)
{
	...
	for (i=0;i<6;i++)
	{
		...
		{
			res.Format("Team %d has a Garrison or Starbase but no home sector",i+1);
			return res;
		}
	}
	for (i=0;i<maxteam;i++)
	{
		if (!(gars[i]))
		{
			res.Format("Team %d need a Garrison or a Starbase",i+1);
			return res;
		}
	}
	if (maxteam <2)
...
{
	IAGCVectorPtr ivector = IAGCVectorPtr(_uuidof(AGCVector));
	if (ivector)
	{
